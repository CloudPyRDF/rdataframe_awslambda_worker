...
def run(mapper, rdf_range, prefix, file_count) -> dict:
    ...
    with monitoring_thread(monitor):
        ...
        filename = serialize_and_upload_to_s3(hist, prefix, N + rdf_range.id, file_count, suffix=suffix)

        if monitoring_on:
            monitoring_results = monitor.get_monitoring_results()
            serialize_and_upload_to_s3(monitoring_results, f'monitor-{prefix}', rdf_range.id, file_count)
    return {
        'statusCode': 200,
